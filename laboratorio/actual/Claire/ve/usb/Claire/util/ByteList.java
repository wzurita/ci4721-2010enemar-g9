...
/*
 * $Id: ByteList.java,v 1.1.1.1 1999/10/31 07:47:02 Paul Exp $
 *
...
/**
 * Handles a list of bytes.
 * @version     $Revision: 1.1.1.1 $
 * @author      Paul Pacheco
 * @since       JDK1.2
 */

public final class ByteList implements Comparable
{
		/**
		...
		private void ensureCapacity(int capacity)
		{
			...
			{
				byte newData[]= new byte[capacity+jumpCapacity];
		 ...
		 */
		public ByteList(int initialCapacity, int jumpCapacity)
		{
		...
		/**
		 * Creates a list of bytes, using the default capacity and grouth
		 */
		public ByteList()
		{
		...
		/**
		 * Creates a list of bytes that is a copy of other
		 ...
		 *         false otherwise
		 */
		public boolean addAll(int index, ByteList other)
		{
			ensureCapacity(size()+other.size());
		 ...
		 *         false otherwise
		 */
		public boolean containsAll( ByteList list)
		{
		...
		public int grow(int newSize)
		{
			...
			Arrays.fill(data, size(), data.length, (byte)0);
		...
		/**
		 * Searches the specified array of bytes for the specified value using the binary search algorithm. The array
		 ...
		 */
		public int binarySearch(byte data)
		{
			int min = 0;
			int max = size()-1;
		...
		{
			ByteList result = new ByteList(toIndex-fromIndex);
		...
		{
			ByteList otherList = (ByteList)otherObject;
